...
import { signHello } from "./hello-sig.js";
...
export class BrokerClient {
  ...
  private async handlePeerFileRequest(msg: { requesterPubkey: string; filePath: string; _reqId?: string }): Promise<void> {
    ...
    // Security: validate input + reject path traversal
    if (!msg.filePath || typeof msg.filePath !== "string" || msg.filePath.length > 4096) {
      sendResponse(undefined, "invalid file path");
      return;
    }
    ...
    // Resolve against shared directories, following symlinks
    ...
    for (const dir of this.sharedDirs) {
      ...
      if (realCandidate.startsWith(realDir + "/") || realCandidate === realDir) {
  ...
  private async handlePeerDirRequest(msg: { requesterPubkey: string; dirPath: string; pattern?: string; _reqId?: string }): Promise<void> {
    ...
    if (typeof dirPath !== "string" || dirPath.length > 4096) {
      sendResponse(undefined, "invalid directory path");
      return;
    }
    if (dirPath.includes("..")) {
      sendResponse(undefined, "path traversal not allowed");
    ...
    try {
      ...
      if (msg.pattern && typeof msg.pattern === "string" && msg.pattern.length <= 200) {
        const escaped = msg.pattern.replace(/[.+^${}()|[\]\\]/g, "\\$&").replace(/\*/g, "[^/]*").replace(/\?/g, ".");
        try { pattern = new RegExp(`^${escaped}$`, "i"); } catch { pattern = null; }
      }
  ...
  private resolveFromMap<T>(
    ...
    if (!entry) {
      if (map.size > 0) {
        this.debug("resolveFromMap: no _reqId match, using FIFO fallback (upgrade broker to fix)");
      }
  ...
  private handleServerMessage(msg: Record<string, unknown>): void {
    ...
    if (msg.type === "push") {
      ...
      this.pushChain = this.pushChain.then(async (): Promise<void> => {
        ...
        // Only attempt base64 fallback for non-direct messages (broadcasts, channels).
        // Direct messages (senderPubkey present) that fail to decrypt MUST stay null
        // to avoid surfacing garbled ciphertext as readable text.
        if (plaintext === null && ciphertext && !senderPubkey) {
          try {
        ...
        while (this.pushBuffer.length > 500) this.pushBuffer.shift();
        if (push.plaintext && push.plaintext.length > 1_000_000) {
          push.plaintext = push.plaintext.slice(0, 1_000_000) + "\n[truncated — message exceeded 1MB]";
        }
